...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = {
    def iter(set: TweetSet, trend: Trending) : Trending =
      if (set.isEmpty) trend
      else iter(set.remove(set.findMin), trend + set.findMin)
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet =
    TweetReader.allTweets.filter((t: Tweet) => google.exists(t.text.contains))

  val appleTweets: TweetSet =
    TweetReader.allTweets.filter((t: Tweet) => apple.exists(t.text.contains))
